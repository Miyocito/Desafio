...
La interfaz está disponible en `app.py` y utiliza exactamente los archivos
`data/a1_input_cases.csv` y `data/historical_transactions.csv`. Presenta la
entrada, ejecuta los cinco agentes, conserva cada salida intermedia y muestra
las métricas de certificación de A5.
...
Configura `GEMINI_API_KEY` en `.env` antes de ejecutar. La UI usa el histórico
en modo lectura: no agrega ni altera transacciones, por lo que las demos son
repetibles. El script `test_a1_a2_a3_a4_a5.py` mantiene su comportamiento
original de añadir los casos A1 al histórico.

```text
CSV A1 → A1 extracción → A2 hipótesis → A3 validación histórica
                                      → A4 regla JSON → A5 backtesting
```
