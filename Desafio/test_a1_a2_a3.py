...
def main() -> None:
    ...
    transcripts = [
        ...
        (
            ...
            "Me llamaron desde un supuesto banco y me indicaron validar un código y me cobraron $700.000 para una compra no reconocida en TechStore.",
        ),
        (
            "CALL-003",
    ...
    transactions = a3.load_transactions_from_csv(Path("data/sample_transactions.csv"))

    print("\n=== SALIDA A3 POR HIPÓTESIS ===")
    if result_a2.patterns:
        for pattern in result_a2.patterns:
            result_a3 = a3.validate_hypothesis(pattern, transactions)
            print(f"\n--- {pattern.pattern_id} ---")
            print(result_a3.model_dump_json(indent=2, exclude_none=False))
    else:
        print("No se generaron hipótesis desde A2, por lo que A3 no recibió patrones para validar.")

    print("\n=== SALIDA A3 AGREGADA ===")
    result_a3_all = a3.validate_hypotheses(result_a2.patterns, transactions)
    print(result_a3_all.model_dump_json(indent=2, exclude_none=False))
